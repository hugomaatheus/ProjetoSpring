	...
	<body>
		...
		<article class="cont_tab" id="Read">
			...
			<form:form  method="post" action="${url}" modelAttribute="funcionario">
				<fieldset id="criar"><legend>Editar Funcionario</legend>
				...
				<br/>
					...
					<form:input id="txttelefone" type="tel" maxlength="15" required="required" path="endereco.telefone" pattern="\([0-9]{2}\)[0-9]{4,6}-[0-9]{3,4}$" title="Digite no seguinte formato (xx) xxxxx-xxxx" />
				<br/>
					<label id="labelemail"><em>Email:</em></label>
					<form:input id="txtemail" type="email" class="input-text" path="email" required="required" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" />
